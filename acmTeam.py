...
In this case, the first team will know all 5 subjects. They are the only team that can be created that knows that many subjects, so
...
and , where is the number of attendees and
...
def acmTeam(topic):
    topic_count = len(topic[0])
    member_count = len(topic)
    members = [set() for _ in range(member_count)]
    for member in range(member_count):
        for s in range(topic_count):
            if topic[member][s] == "1":
                members[member].add(s)
    teams = []

    for i in range(member_count):
        for j in range(i, member_count):
            if i == j:
                continue
            teams.append(len(members[i] | members[j]))
